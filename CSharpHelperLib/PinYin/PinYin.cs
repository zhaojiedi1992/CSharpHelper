...
namespace CSharpHelperLib.PinYin
{
    public class PinYin
    {
         ...
         "鳕鳗鳘鳙鳜鳝鳟鳢靼鞅鞑鞒鞔鞯鞫鞣鞲鞴骱骰骷鹘骶骺骼髁髀髅髂髋髌髑魅魃魇魉魈魍魑飨餍餮饕饔髟髡髦髯髫髻髭髹鬈鬏鬓鬟鬣麽麾縻麂麇麈麋麒鏖麝麟黛黜黝黠黟黢黩黧黥黪黯鼢鼬鼯鼹鼷鼽鼾齄";
            ...
            for (int i = 0; i < hz.Length; i++)
            {
                ...
                else if (array[0] >= 176 && array[0] <= 215)
                {
                    ...
                    if (String.Compare(hz[i].ToString(), "匝", StringComparison.Ordinal) >= 0)
                        returnPy += "z";
                    else if (String.Compare(hz[i].ToString(), "压", StringComparison.Ordinal) >= 0)
                        returnPy += "y";
                    else if (String.Compare(hz[i].ToString(), "昔", StringComparison.Ordinal) >= 0)
                        returnPy += "x";
                    else if (String.Compare(hz[i].ToString(), "哇", StringComparison.Ordinal) >= 0)
                        returnPy += "w";
                    else if (String.Compare(hz[i].ToString(), "塌", StringComparison.Ordinal) >= 0)
                        returnPy += "t";
                    else if (String.Compare(hz[i].ToString(), "撒", StringComparison.Ordinal) >= 0)
                        returnPy += "s";
                    else if (String.Compare(hz[i].ToString(), "然", StringComparison.Ordinal) >= 0)
                        returnPy += "r";
                    else if (String.Compare(hz[i].ToString(), "期", StringComparison.Ordinal) >= 0)
                        returnPy += "q";
                    else if (String.Compare(hz[i].ToString(), "啪", StringComparison.Ordinal) >= 0)
                        returnPy += "p";
                    else if (String.Compare(hz[i].ToString(), "哦", StringComparison.Ordinal) >= 0)
                        returnPy += "o";
                    else if (String.Compare(hz[i].ToString(), "拿", StringComparison.Ordinal) >= 0)
                        returnPy += "n";
                    else if (String.Compare(hz[i].ToString(), "妈", StringComparison.Ordinal) >= 0)
                        returnPy += "m";
                    else if (String.Compare(hz[i].ToString(), "垃", StringComparison.Ordinal) >= 0)
                        returnPy += "l";
                    else if (String.Compare(hz[i].ToString(), "喀", StringComparison.Ordinal) >= 0)
                        returnPy += "k";
                    else if (String.Compare(hz[i].ToString(), "击", StringComparison.Ordinal) >= 0)
                        returnPy += "j";
                    else if (String.Compare(hz[i].ToString(), "哈", StringComparison.Ordinal) >= 0)
                        returnPy += "h";
                    else if (String.Compare(hz[i].ToString(), "噶", StringComparison.Ordinal) >= 0)
                        returnPy += "g";
                    else if (String.Compare(hz[i].ToString(), "发", StringComparison.Ordinal) >= 0)
                        returnPy += "f";
                    else if (String.Compare(hz[i].ToString(), "蛾", StringComparison.Ordinal) >= 0)
                        returnPy += "e";
                    else if (String.Compare(hz[i].ToString(), "搭", StringComparison.Ordinal) >= 0)
                        returnPy += "d";
                    else if (String.Compare(hz[i].ToString(), "擦", StringComparison.Ordinal) >= 0)
                        returnPy += "c";
                    else if (String.Compare(hz[i].ToString(), "芭", StringComparison.Ordinal) >= 0)
                        returnPy += "b";
                    else if (String.Compare(hz[i].ToString(), "啊", StringComparison.Ordinal) >= 0)
                        returnPy += "a";
                }
                //二级汉字
                else if (array[0] >= 215)
                {
                    returnPy += ls_second_eng.Substring(ls_second_ch.IndexOf(hz[i].ToString(), 0, StringComparison.Ordinal), 1);
                }
            }
